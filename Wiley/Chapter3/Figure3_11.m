%----------------------------------------------------------------
% This code can be used to generate Figure 3.11
%----------------------------------------------------------------
clear all
close all
...
TT=(T-abs(tau));
dummy=k*(ones(length(fd),1)*tau)-fd*ones(1,length(tau));
X1=sinc(dummy.*(ones(length(fd),1)*TT));
p=find(TT<0);
TT(p)=0;
...
%---Figure 3.11(b)------------------------------------------------
imagesc(tau*1e3,fd*1e-3,abs(X(length(X):-1:1,:)));
colormap(gray);
colorbar
set(gca,'FontName', 'Arial', 'FontSize',14,'FontWeight','Bold');
xlabel ('Time Delay [ms]')
